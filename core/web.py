...
def run_recon_pipeline(target_domain, target_reports_dir, subdomains_file, nuclei_path, katana_path, ferox_path):
    ...
    # 2. Nuclei Execution
    run_command(
        ["nuclei", "-l", subdomains_file, "-json-export", nuclei_path],
        "Vulnerability Scan (Nuclei)"
    )

    # 3. Katana Execution - Configurado com -jc para formato estruturado limpo
    run_command(
        ["katana", "-list", subdomains_file, "-jc", "-o", katana_path],
        "Web Crawling (Katana)"
    )

    # 4. Feroxbuster Execution
    print("[+] Launching subprocess: Directory Brute Force (Feroxbuster)...")
    try:
        with open(subdomains_file, "r") as stdin_source:
            subprocess.run(
                ["feroxbuster", "--stdin", "-o", ferox_path],
                stdin=stdin_source, check=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE
            )
    except Exception as e:
        print(f"[!] Feroxbuster pipeline encountered an issue: {e}")
